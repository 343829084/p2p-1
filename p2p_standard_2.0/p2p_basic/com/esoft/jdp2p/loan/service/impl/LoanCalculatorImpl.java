...
import com.esoft.jdp2p.invest.InvestConstants.InvestType;
...
public class LoanCalculatorImpl implements LoanCalculator {
	...
	public Double calculateMoneyNeedRaised(String loanId)
		...
		List<Object> investMoney = ht
				.find("select sum(invest.investMoney) from Invest invest where invest.loan.id=? and invest.status !=? and invest.type !=?",
						new String[] { loanId,
						InvestConstants.InvestStatus.CANCEL ,InvestConstants.InvestType.COMPANYINVEST});
...
//		remain = remain < loan.getMaxInvestMoney() ? remain :loan.getMaxInvestMoney();
	...
	public Long countSuccessInvest(String loanId) {
		Object o = ht
				.find("select count(im) from Invest im where im.loan.id = ? and  im.status in (?,?,?,?,?) and type != ? ",
						new String[] { loanId,
								...
								InvestConstants.InvestStatus.REPAYING ,InvestConstants.InvestType.COMPANYINVEST}).get(0);
		if (o == null) {
			return 0L;
		}
		return (Long) o;
