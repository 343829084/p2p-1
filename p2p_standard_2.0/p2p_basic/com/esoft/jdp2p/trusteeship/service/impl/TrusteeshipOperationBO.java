...
public class TrusteeshipOperationBO {
	...
	public List<TrusteeshipOperation> getUnCallbackOperations(int minute){
		...
		return ht.find(hql, new Object[]{TrusteeshipConstants.Status.SENDED, date });
	}
	...
	public void save(TrusteeshipOperation to) {
		...
		if (StringUtils.isEmpty(to.getOperator())) {
			throw new IllegalArgumentException(
					"trusteshipOperation.operator can not be empty!");
		}
...
//		ht.bulkUpdate(
//				"delete TrusteeshipOperation to where to.type=? and to.markId=? and to.operator=? and to.trusteeship=?",
//				new String[] { to.getType(), to.getMarkId(), to.getOperator(),
//						to.getTrusteeship() });
		ht.save(to);
	}
	 ...
	 * @return 操作对象
	 */
	public TrusteeshipOperation get(String type, String markId, String trusteeship) {
