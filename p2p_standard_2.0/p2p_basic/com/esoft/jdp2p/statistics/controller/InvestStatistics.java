...
public class InvestStatistics {
	...
	public List<InvestPulished> getIps(final Date startTime ,final Date endTime) {
		...
		final String hql = "SELECT invest.user.id, SUM(ir.corpus), SUM(ir.interest) ,invest.user.photo  FROM InvestRepay ir where ir.invest.time >= ? and ir.invest.time <= ? GROUP BY ir.invest.user ORDER BY SUM(ir.corpus) desc";
		@SuppressWarnings("unchecked")
		List<Object[]> objs = ht
		...
		if (objs.size() > 0) {
			for (Object obj : objs) {
				...
				InvestPulished ip = new InvestPulished((String) objs2[3],(String) objs2[0],
