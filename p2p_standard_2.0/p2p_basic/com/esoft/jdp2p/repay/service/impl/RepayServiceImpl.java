...
public class RepayServiceImpl implements RepayService {
		...
		for (InvestRepay ir : irs) {
			...
			if (isWealth(ir.getInvest().getId())) {
				if (!wealthRepay(ir,repay)) {
					throw new NormalRepayException("还款：" + repay.getId() + " 正常还款状态，好友钱代投收益转账失败 。");
				}
				continue;
			}
			...
			// 投资者手续费
			userBillBO.transferOutFromBalance(userId,
	...
	public boolean isWealth(String inverIdString){
		List<InvestRepay> irs = ht
				.find("from Invest in where in.investId=?",inverIdString);
	...
	private boolean wealthRepay(InvestRepay ir, LoanRepay repay) throws InsufficientBalance {
		...
		userWealthBO.transferIntoBalance(
				userId,
				ArithUtil.add(ir.getCorpus(), ir.getInterest()),
				OperatorInfo.NORMAL_REPAY,
				"投资：" + ir.getInvest().getId() + "收到还款, 还款ID:"
						...
						+ ir.getCorpus() + "  利息：" + ir.getInterest());
		// 投资者手续费
		userWealthBO.transferOutFromBalance(userId,
				ir.getFee(), OperatorInfo.NORMAL_REPAY, "投资："
						+ ir.getInvest().getId() + "收到还款，扣除手续费, 还款ID:"
						+ repay.getId());
		systemBillService.transferInto(ir.getFee(),
				OperatorInfo.NORMAL_REPAY, "好友钱投资：" + ir.getInvest().getId()
						+ "收到还款，扣除手续费, 还款ID:" + repay.getId());
	...
	public void advanceRepay(String loanId) throws InsufficientBalance,
		...
		userBillBO.transferOutFromBalance(loan.getUser().getId(), sumPay,
				OperatorInfo.ADVANCE_REPAY, "借款：" + loan.getId() + "提前还款，本金："
						+ sumCorpus + "，用户罚金：" + feeToInvestor + "，系统罚金："
						+ feeToSystem + "，借款手续费：" + feeAll);
		// 余额不足，抛异常
	...
	public boolean isInRepayPeriod(Date repayDate) {
		repayDate = DateUtil.StringToDate(DateUtil
				.DateToString(repayDate, DateStyle.YYYY_MM_DD_CN));
		Date now = new Date();
