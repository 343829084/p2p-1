package com.esoft.jdp2p.withdraw;

import java.io.FileNotFoundException;
import java.io.IOException;
import java.util.Properties;

public class CapitalPoolConstans {
	...
	static {
		...
		try {
			props.load(Thread.currentThread().getContextClassLoader()
					.getResourceAsStream("capitalPool.properties"));
		} catch (FileNotFoundException e) {
			throw new RuntimeException("找不到capitalPool.properties文件", e);
		} catch (IOException e) {
			throw new RuntimeException("读取capitalPool.properties文件出错", e);
		}
	}
