...
public class RechargeServiceImpl implements RechargeService {
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public void rechargePaySuccessWealth(String rechargeId) {
		Recharge recharge = ht
				.get(Recharge.class, rechargeId);
		ht.evict(recharge);
		recharge = ht
		...
		if (recharge != null
				&& recharge.getStatus().equals(
						UserConstants.RechargeStatus.WAIT_PAY)) {
			rechargeBO.rechargeSuccessOfWealth(recharge);
		}
	...
	public String createRechargeOrder(Recharge recharge) {
		...
		recharge.setStatus(UserConstants.RechargeStatus.WAIT_PAY);
		ht.save(recharge);
		String aString=FacesUtil.getHttpServletRequest().getContextPath()
...
//		bcs.add(new RechargeBankCardImpl("CMB", "招商银行"));
	...
	@Override
	@Transactional(rollbackFor=Exception.class)
	public void rechargeByAdmin(UserWealth userBill) {
		Recharge r = new Recharge();
		r.setActualMoney(userBill.getMoney());
		r.setCoupon(null);
		r.setFee(0D);
