	...
	public void autoInvest(String loanId) {
		...
		for (int i = 0; i < ais.size(); i++) {
			...
			if (log.isDebugEnabled()) {
				...
				log.debug("autoInvest maxDeadline: "
						+ ai.getMaxDeadline()
						+ "$minDeadline: " + ai.getMinDeadline()+"$loanDeadline: "+loan.getDeadline());
				log.debug("autoInvest maxRate: "
						+ ai.getMaxRate()
						+ "$minRate: " + ai.getMinRate()+"$loanRate: "+loan.getRate());
			}
...
//				2、投标进度达到 {s}%时停止自动投标， 若剩余自动投标金额小于用户设定的每次投标金额，也会进行投标，投资金额向下取该标剩余自动投标金额。
