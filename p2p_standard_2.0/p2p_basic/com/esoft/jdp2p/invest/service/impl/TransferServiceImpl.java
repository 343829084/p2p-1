...
public class TransferServiceImpl implements TransferService {
	...
	public void transfer(String transferApplyId, String userId,
		...
		//总转让本金占持有本金的本例
		double allTransferCorpusRate = ArithUtil.div(ta.getCorpus(), ta.getInvest().getMoney());
		...
		userBillBO.transferOutFromBalance(userId, buyPrice,
				OperatorInfo.TRANSFER, "购买债权，编号：" + investNew.getId());
...
//			    还款本金*转让本金比例*购买本金比例
				irNew.setCorpus(ArithUtil.mul(ir.getCorpus(),allTransferCorpusRate, 2));
				irNew.setDefaultInterest(ArithUtil.mul(ir.getInterest(),allTransferCorpusRate,2));
				irNew.setFee(ArithUtil.mul(ir.getFee(),allTransferCorpusRate, 2));
				irNew.setInterest(ArithUtil.mul(ir.getInterest(),allTransferCorpusRate, 2));
