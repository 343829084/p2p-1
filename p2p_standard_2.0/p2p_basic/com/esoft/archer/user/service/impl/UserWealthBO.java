...
@Service(value = "userWealthBO")
public class UserWealthBO {

	@Resource
	private HibernateTemplate ht;

	private UserWealth getLastestBill(String userId) {
		DetachedCriteria criteria = DetachedCriteria.forClass(UserWealth.class);
		criteria.addOrder(Order.desc("seqNum"));
		criteria.setLockMode(LockMode.UPGRADE);
		criteria.add(Restrictions.eq("user.id", userId));
		List<UserWealth> ibs = ht.findByCriteria(criteria, 0, 1);
		if (ibs.size() > 0) {
			UserWealth ub = ibs.get(0);
	...
	public void freezeMoney(String userId, double money, String operatorInfo,
		...
		UserWealth ibLastest = getLastestBill(userId);
		UserWealth ib = new UserWealth();
	...
	public double getBalance(String userId) {
		UserWealth ub = getLastestBill(userId);
	...
	private double getSumByType(String userId, String type) {
		String hql = "select sum(ub.money) from UserWealth ub where ub.user.id =? and ub.type=?";
