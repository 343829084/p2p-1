...
public class FileUploadFilenameExtensionValidation implements PhaseListener {
	...
	public void beforePhase(PhaseEvent event) {
		...
		HttpServletRequestWrapper requestWarpper = (HttpServletRequestWrapper) fc
				.getCurrentInstance().getExternalContext().getRequest();
		HttpServletRequest request = (HttpServletRequest) requestWarpper
				.getRequest();
		if (request instanceof MultipartRequest) {
			...
			for (FileUpload fileUpload : fileUploads) {
				...
				FileItem file = ((MultipartRequest) request)
						.getFileItem(fileUpload.getClientId());
				if (file == null) {
					continue;
				}
