package com.esoft.ipspay.withdraw.service.impl;
...
@Service
public class IpsPayWithdrawOperation extends
		IpsPayOperationServiceAbs<WithdrawCash> {
...
//						pErrCode + ":" + pErrMsg));
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public TrusteeshipOperation createOperation(WithdrawCash wc,
			FacesContext facesContext) {
		try {
			wc.setType(configService.getConfigValue("withdraw_cap"));
			withdrawCashService.applyWithdrawCash(wc);
		...
		String feeType = "1";
		try {
			feeType = configService.getConfigValue("ipspay.withdraw_fee_type");
		...
		TrusteeshipOperation to = createTrusteeshipOperation(wc.getId(),
				...
				TrusteeshipConstants.OperationType.WITHDRAW_CASH, params);
