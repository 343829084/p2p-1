...
public class MyLoanRequestHandlerImpl implements RequestHandler{
	...
	public Out handle(In in, Out out) {
		...
		try{
			...
			if(op!=null&&op.length()>0){
				GsonBuilder builder=new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").setExclusionStrategies(new LoanExclusionStrategy()).registerTypeAdapterFactory(HibernateProxyTypeAdapter.FACTORY);
				...
				out=getLoans(out,session,builder,userId,curPage,size,statusList);
	...
	private Out getLoans(Out out,Session session,GsonBuilder builder,String userId,int curPage,int size,List<String> statusList) throws IllegalAccessException, InvocationTargetException, NoMatchingObjectsException{
		...
		if(list!=null&&list.size()>0){
			...
			for(int i=0;i<list.size();i++){
				LoanSub sub=LoanUtil.getLoanSub(builder, list.get(i), loanCalculator);
				subList.add(sub);
			}
